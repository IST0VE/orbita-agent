...
def isolated_paths(clean_env, monkeypatch: pytest.MonkeyPatch, tmp_path):
    ...
    # Журнал операций Jira — тоже файл, и тоже свой на каждый тест. Общий
    # журнал помнил бы отправки предыдущего прогона тестов, и тест, который
    # заводит задачи, получал бы «уже заведено» от чужой записи.
    monkeypatch.setenv("JIRA_JOURNAL_PATH", str(tmp_path / "jira-operations.sqlite3"))
