...
flowchart TB
    ...
    INV -->|"до 3 инструментов за ответ"| TOOLS["additional_tools — код<br>11 read-only инструментов: метрики в границах scope,<br>текущие pods и события, статус теста, Jira, Confluence"]
    TOOLS -->|"не более 12 000 знаков на результат"| INV
    INV -->|"не более 4 вызовов модели,<br>дальше — бюджет и таймаут"| FIN{{"final_analysis — код проверяет ответ модели"}}
...
Перед записью отчёта граф останавливается сам: publisher показывает подготовленный
документ и ждёт решения оператора. Без подтверждения не публикуется ничего.
