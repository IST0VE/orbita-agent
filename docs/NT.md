...
Известное отклонение: гипотезы ломают разметку отчёта. Блок наблюдений
приклеивается к строке `Проверенные наблюдения:`, открывающий забор оказывается
не в начале строки и блоком кода не становится, а его закрывающий забор открывает
следующий. Чётность сбивается на каждой гипотезе, и в storage-разметке
Confluence заголовки `## Evidence`, `## Baseline comparison`, `## Sources` и
пояснения к разделам уезжают внутрь блоков кода. В портале этого не видно по
другой причине: `SafeMarkdown` знает только заголовки, ссылки и абзацы, поэтому
все JSON-блоки и таблица расхода токенов там и так показаны обычным текстом
вместе с тройными кавычками.
