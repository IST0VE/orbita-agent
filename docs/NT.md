...
Для подготовки сценария и проведения нового теста используйте отдельный
[граф `nt_run`](NT_RUN.md).
...
| Поле | Что означает |
| --- | --- |
| `execution_status` | `COMPLETED`, `STOPPED`, `FAILED` или `UNKNOWN`: как закончился сам тест |
| `analysis_result` | `PASSED`, `FAILED` или `INCONCLUSIVE` по SLA целевого сервиса |
| `sla_verdict` | Тот же вердикт с происхождением: сервис, окружение, период, источники, проверенные метрики, причины |
...
Три первых поля отвечают на три разных вопроса: выполнился ли тест, уложился ли он
в SLA и хватило ли данных, чтобы это утверждать. Прерванный тест не получает `PASSED`
ни при каких рядах — это отдельная причина в `sla_verdict.reasons`, а не нарушение порога.
...
`sla_verdict.basis` всегда равен `whole_run_maxima`: проверяются максимумы рядов за
заявленный период целиком, включая разгон и хвост. Устойчивость плато это не доказывает —
для неё есть отдельный расчёт `capacity_assessment`, и подменять один другим нельзя.

Симулированная телеметрия положительным вердиктом не становится. Перечислите такие
источники в `NT_SIMULATED_SOURCES` (например, `prometheus` в конфигурации стенда):
их ряды не измеряют реакцию цели на прогон, поэтому `PASSED` по ним превращается
в `INCONCLUSIVE` с причиной, а наблюдаемая устойчивая RPS не показывается вовсе.
Нарушение порога при этом остаётся нарушением: отказ симулятор подтвердить может.

Вердикт вычисляется кодом и не переопределяется текстом модели. Формулировка «тест
пройден» в разделе гипотез или в выводе планировщика не меняет `analysis_result`;
при расхождении верны числа.
...
Известное отклонение: если лимиты разобраны из текста Jira или Confluence, а
оператор не задал `sla_comparators` в запросе, строгий `<` теряется и сравнение
идёт по `<=`. Пустой `sla_comparators` разбора затирает операторы, найденные
в тексте, после чего `metadata.hydrate` не возвращает и операторы gateway:
лимиты пришли не от него. До исправления задавайте `<` явным JSON.

Точка доли вне 0..1 отбрасывается как невалидная: контракт единиц не допускает
значения выше 1. На стенде одной такой точки (`cpu` order-service = 1,022, CPU
выше лимита cgroup) хватает, чтобы `diagnostic_status` стал `PARTIAL`, а ряд
попал в `diagnostic_gaps`. Это нормальный burst, а не порча данных: обрезайте
долю сверху в серверном выражении, иначе полнота диагностики будет проседать
на каждом насыщенном прогоне.
...
Начальный и финальный JSON ограничены `NT_BRIEF_TOKENS` (по умолчанию 12 000
токенов, около 48 000 знаков), ответ одного инструмента — `NT_TOOL_RESULT_TOKENS`
(2 000). Сокращение брифа не удаляет полный журнал улик из состояния и отчёта.
Сырые точки освобождаются после расчётов.

Эти два потолка подгоняют вход под окно модели. Постоянная часть вызова —
промпт роли и схемы тринадцати инструментов — занимает около 1 800 токенов;
всё остальное место делят бриф и накопленная переписка. На примере
[nt-run-2291](examples/nt-run-2291.md) вызов целиком весит 13 400 токенов при
значениях по умолчанию, 7 700 при `NT_BRIEF_TOKENS=6000` и 3 900 при `2500`
(23 улики из 147). Модели с окном 7 000 токенов подходит связка
`NT_BRIEF_TOKENS=2500`, `NT_TOOL_RESULT_TOKENS=500` и `LLM_MAX_HISTORY_TOKENS=4000`.

Урезанный бриф — это меньше улик у модели, а не другой вердикт: `analysis_result`,
полноту диагностики, плато и отчёт считает код. Сколько улик не поместилось,
написано в самом брифе (`omitted_evidence`), и модели там же сказано не делать
выводов об отсутствующих данных. Числа в брифе округляются до четырёх значащих
цифр — до тех же, что человек видит в отчёте.
...
`possible` на практике встречается редко: любое заполненное поле
`counter_evidence_ids` опускает уверенность до `unknown`, а модель заполняет его
охотно — в прогоне по стенду так вышло у всех пяти принятых гипотез, включая ту,
где одна и та же улика стояла и в подтверждающих, и в противоречащих. Код такую
пару не отбраковывает, и в отчёте она видна как есть.
...
Дедлайн проверяется на входе в узел и начатый вызов модели не прерывает, а
`LLM_TIMEOUT_S` покрывает только ввод-вывод httpx. На Windows узел `investigate`
однажды простоял десять минут внутри `CertGetCertificateChain` — проверки цепочки
сертификатов, которую делает `truststore`; `langgraph-api` включает его глобально
при импорте (`DISABLE_TRUSTSTORE`). Это не сокетная операция, и ни один таймаут
её не ограничивает: портал показывает «Выполняется» без конца. «Остановить»
возвращает управление и уже посчитанный детерминированный вердикт, но
заблокированный рабочий поток живёт до перезапуска backend. Срыв разовый —
повторные вызовы к тому же провайдеру уходили за две-три секунды.

Наблюдения, улики, аномалии, timeline и сравнения выводятся таблицами: строка
на наблюдение вместо дампа состояния. Раньше приложение `Evidence` занимало
больше половины отчёта, а блок наблюдений приклеивался к строке
`Проверенные наблюдения:` — открывающий забор оказывался не в начале строки,
чётность сбивалась на каждой гипотезе, и в storage-разметке Confluence заголовки
`## Evidence`, `## Baseline comparison` и `## Sources` уезжали внутрь блоков
кода. Теперь блоков кода в гипотезах нет. Улики по сервисам вне фокуса анализа
сворачиваются счётчиком, а всё, на что ссылается гипотеза, остаётся в таблице:
наблюдение по `evidence_id` из отчёта находится всегда.
...
2. Из окружения backend выполните `python scripts/diagnose_nt.py --test-id <id> --check-metrics`.
   Размер запроса к модели проверяет та же команда с `--model-input --window <окно модели>`:
   она ничего не читает и не вызывает модель, а печатает постоянную часть вызова,
   первый и худший запрос и — при заданном окне — значения трёх потолков под него.
...
Зелёный прогон всей проверки (989 тестов) не заменяет прогон по стенду: оба
отклонения выше — потерянные компараторы и разметка гипотез — он проходит.
...
Стенд воспроизводит не весь контур. `nt-run-2291` на нём даёт `FAILED` и
`diagnostic_status=PARTIAL`, а устойчивую нагрузку не называет вовсе
(`INCONCLUSIVE`): Prometheus стенда объявлен в `NT_SIMULATED_SOURCES`, и
симулированные ряды плато не подтверждают. Вердикт и ранжирование повторяются
от запуска к запуску. Не показывает он вот что:

- сравнение прогонов. `nt-sim` не отдаёт `environment_fingerprint`, а для
  предыдущего теста он берётся только из gateway, поэтому `previous_comparison`
  всегда `NOT_COMPARABLE`: сопоставление ступеней и регрессии на стенде
  не проверяются, а `nt-run-2187` вдобавок идёт по другому сценарию;
- поиск по контексту. Моки Jira и Confluence сравнивают CQL и JQL подстрокой,
  и естественный запрос модели не находит даже существующую страницу;
- metadata метрик: `discover_metrics` возвращает пустые `type` и `help`.

Пойдёт ли модель за собственным запросом, стенд не гарантирует: в одном прогоне
она обошлась discovery и поиском, в другом составила два запроса по
`http_requests_in_flight`. Отказ оператора доходит до неё как `POLICY_DENIED`
с текстом причины, и прогон продолжается; отказ от публикации оставляет
`REJECTED` и не создаёт документа. Прежние прогоны по стенду стоили около двух
центов: 2–4 вызова модели, 44–89 тысяч входных токенов при cache hit 52–77%.
Прогон 24 сентября 2026 года на демо-шлюзе с qwen3.8-flash — 6 вызовов, 148
тысяч входных и 42 тысячи выходных токенов (почти всё — скрытые рассуждения
модели) и около получаса из-за квоты шлюза.

[Пример отчёта](examples/nt-run-2291.md) и снимки ниже пересобраны
`shoot_nt_portal.py` по этому прогону. Модель в нём за собственным запросом
не пошла, поэтому кадра подтверждения запроса нет. Первым в ранжировании стоит
`order-service` (0,989), вторым — ловушка `image-resizer` (0,906). Ранжирование
её не отсеивает; отсеивает разбор причин: все четыре гипотезы отчёта — об
`order-service`.

Во время прогона вердикт, полнота диагностики и ранжирование видны во вкладке
«Результат», пока модель ещё разбирает причины:

![Итоги во вкладке «Результат» во время прогона](image/NT/portal-run.png)

Перед публикацией — подтверждение с готовым отчётом:

![Подтверждение публикации отчёта](image/NT/portal-approval.png)

Опубликованный отчёт открывается вкладкой «Документ»; гипотезы — в разделе
Root cause analysis:

![Отчёт во вкладке «Документ»](image/NT/portal-report.png)
![Раздел Root cause analysis](image/NT/portal-hypotheses.png)

## Границы поставки

Граф `nt` отвечает за **анализ завершённых тестов**. Его HTTP-адаптер метаданных
не управляет нагрузкой: методы управления возвращают `POLICY_DENIED`.
Подготовка сценария, пробный и основной прогоны, мониторинг и остановка реализованы
в отдельном [графе `nt_run`](NT_RUN.md) через локальный k6 runner. Менять
интенсивность уже запущенного теста этот контур не позволяет: следующий эксперимент
требует нового плана. `integrations/logs.py` предоставляет только
