...
Стенд воспроизводит не весь контур. `nt-run-2291` на нём даёт `FAILED`,
`diagnostic_status=PARTIAL` и устойчивую нагрузку 1764,8 RPS при
`NT_STABLE_SECONDS=180`; вердикт, плато и ранжирование повторяются от запуска
к запуску. Не показывает он вот что:
...
`REJECTED` и не создаёт документа. Прогон по стенду стоит около двух центов:
2–4 вызова модели, 44–89 тысяч входных токенов при cache hit 52–77%.

[Пример отчёта](examples/nt-run-2291.md) и снимки ниже сняты до `nt-analysis-v3`:
в них нет разделов Diagnostic completeness, Load phases и Analysis policy, другая
устойчивая нагрузка (1829,8 RPS) и другой список подозреваемых. Эталоном текущих
расчётов и confidence они не являются — пересоберите их `shoot_nt_portal.py`.
