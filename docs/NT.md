...
При некорректном входе `precheck → report`. LLM не назначает итоговый статус и
...
и не вызывает LLM повторно. Публикация комментария в Jira в этот MVP не входит.
