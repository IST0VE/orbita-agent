...
![Папка partial-refund с двумя отмеченными документами](assets/screenshots/inputs.png)

*Выбрана папка `partial-refund`, в ней отмечены встреча и письмо финансов: галочки в дереве, карточки в «Параметрах прогона», «2 файла» в строке контекста над полем задачи и имена в правой колонке. Без отметок в «Параметрах прогона» стояло бы «не выбран: конвейер прочитает папку целиком».*

## Ограничение объёма

`AGENT_INPUT_MAX_CHARS=20000` действует в двух смыслах, и различать их важно.

| Что читается | Как применяется потолок |
| --- | --- |
| Один документ: инструмент роли, предпросмотр в интерфейсе | На файл. При превышении читается начало с пометкой об обрезке |
| Комплект материалов: файлы папки задачи, источники по ссылкам, основной документ и новые материалы при обновлении | На весь набор. Файлы читаются по порядку, пока остаётся место; непрочитанные перечисляются отдельно |

`AGENT_INPUT_MAX_CHARS=0` означает отсутствие ограничения в обоих случаях: материалы читаются целиком. Это не «ноль символов» — непустая папка задачи не превращается в отказ «ни один файл не прочитан».
