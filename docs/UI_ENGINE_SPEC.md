...
- `/api/ui/actions/validate` проверяет запрос, но не помечает его выполненным: после ошибки отправки валидацию можно повторить;
- SDK отправляет `Command(resume={interrupt_id: payload})` с настоящим ID остановки LangGraph и `multitaskStrategy: reject`; старый ответ не должен подтверждать следующую остановку;
- форма следует актуальному snapshot SDK и остаётся доступной после неудачной отправки;
- повторная отправка одного ответа должна быть идемпотентной; это свойство даёт адресация, а не счётчик в UI;
- ответ на уже разрешённую остановку LangGraph применять не к чему: UI обновляет snapshot, а не предлагает отправить снова;
- `idempotency_key` в preflight — сквозной идентификатор для журнала, а не признак выполнения;
- при нескольких interrupts отображается очередь или набор карточек согласно server semantics (не реализовано: адресуется первая остановка).
