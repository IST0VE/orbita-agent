...
После редактирования `.env` перезапустите backend: файл читается один раз при старте процесса. Что применено прямо сейчас, видно в окне настроек — блок «Применено сейчас» показывает значение, его источник (файл или окружение процесса) и отметку «нужен перезапуск» там, где файл разошёлся с процессом. Значения секретов там замаскированы.
