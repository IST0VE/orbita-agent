...
Для анализа НТ дополнительно настройте [источники метрик](NT.md). Для проведения нового НТ нужны [runner, k6 и разрешённый стенд](NT_RUN.md). Команды первого запуска выше поднимают интерфейс и backend; тестируемый сервис и runner они не запускают.
...
| Видны сценарии из `langgraph.json` и папки                                    | Frontend получил данные API                                                                                                          |
