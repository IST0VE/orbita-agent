...
Откройте `.env` в редакторе. Для первого прогона приведите эти строки к такому виду. Две из них отличаются от шаблона намеренно — `PUBLISH_TARGET` и `JIRA_CREATE_ISSUES`: первый прогон сохраняет документы файлами и не трогает трекер.
...
![Первое открытие Orbita: сценарий «Архитектурная аналитика», материалы ещё не выбраны](assets/screenshots/start.png)

*Первое открытие: вверху переключатель сценария и связь с сервером, слева материалы задачи, в центре схема конвейера и поле задачи, справа — что делает сценарий и что для него выбрано. [Как пересобрать кадр](VISUALS.md).*

## 5. Получите первый результат

1. Выберите «Архитектурная аналитика».
2. Нажмите «Новый прогон», если открыт предыдущий контекст.
...
После редактирования `.env` перезапустите backend: файл читается один раз при старте процесса. Что применено прямо сейчас, видно в настройках, в разделе «Продвинутые», — блок «Применено сейчас» показывает значение, его источник (файл или окружение процесса) и отметку «нужен перезапуск» там, где файл разошёлся с процессом. Значения секретов там замаскированы.

Переменная, заданная в окружении процесса (терминал, `docker compose`, systemd), сильнее файла: правка `.env` её не изменит ни сейчас, ни после перезапуска. Блок «Применено сейчас» называет такой источник прямо.

Файлы в `published/` сохраняются независимо от остановки процессов. Не удаляйте `.langgraph_api/`, если рассчитываете продолжить текущие треды.
