...
Откройте `http://localhost:5173`, выберите **«Проведение нагрузочного тестирования»**, нажмите «Новый прогон» и отправьте:
...
Снимки ниже сняты 24 сентября 2026 года скриптом `shoot_nt_run_portal.py` по настоящему прогону на стенде. Пробный запрос k6 в нём до цели не дошёл: единственный запрос `health check` оборвался через 10 секунд, и кампания закончилась без основного теста, поэтому кадра с живыми метриками нет. Как выглядит удачный прогон, показывает [стенограмма ниже](#как-читать-сохранённый-пример).
...
Раскройте «Сводку конвейера»: предмет подтверждения — конкретный план, данные и скомпилированный скрипт. На кадре — второе разрешение того же треда: после неудачного пробного прогона планировщик предложил диагностический повтор на одном виртуальном пользователе.

![План и k6-скрипт](image/NT_RUN/portal-launch-plan.png)

Во время теста во вкладке «Результат» обновляются `test_id`, статус и последнее пятисекундное окно метрик. Итоговый отчёт открывается из «Опубликованных документов» вкладкой «Документ»; в нём результаты прогонов и, если основной тест прошёл, отдельный исторический анализ:
...
Для нового эксперимента начните «Новый прогон» и проверьте новый план. Не нужно воспроизводить исторические числа в точности: оценивайте корректность сценария, реальные статусы, происхождение измерений и достаточность данных для вывода.
