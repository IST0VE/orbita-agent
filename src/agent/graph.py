...
Что делает каждая нода:
  ...
  tools     чтение файлов задачи. В этом конвейере — только для первой роли:
            ...
            материалами. Сейчас так во всех конвейерах: спрашивающая роль
            ровно одна (`Role.reads_files`). Узел тем не менее общий, и возврат
            из него адресуется по `state["stage"]`, а не зашитым ребром: вторая
            такая роль добавляется флагом и не требует правки топологии. Ходы
            в инструменты ограничены потолком TOOL_TURNS_PER_RUN — решение
            «спросить ещё раз» принимает модель, и без потолка петля упирается
            только в recursion_limit LangGraph. Набор инструментов свой у
            каждого конвейера (`pipeline.Pipeline.tools`, `tools.py`);
