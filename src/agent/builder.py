...
def build_graph(
    ...
    # один на граф, а спрашивать его могут несколько ролей — у конвейера
    # подготовки задачи их две, — поэтому адресат берётся из состояния, а не
    # зашивается в ребро. Список назначений всё равно перечислен целиком:
    # LangGraph рисует граф по нему, а не по тому, что вернёт функция.
