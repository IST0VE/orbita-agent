...
при старте Python-процесса, а клиент LLM также создаётся при импорте графа,
поэтому изменения файла через интерфейс применяются после перезапуска сервера.
