...
#
# Менять модель между ходами одного треда по-прежнему нельзя — сменится
# префикс и обнулится кеш провайдера. Кеш по ключу это и сохраняет: модель
# меняется между тредами, а не внутри одного.
