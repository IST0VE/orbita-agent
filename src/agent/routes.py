...
def make_role_router(
    ...
    """
    Куда после роли: в остановку, в инструменты или на следующий этап.

    Веток у роли без инструментов две, а не одна, и вторая появилась вместе с
    паузой (`pause.py`). Остановка оператора может теперь случиться на любом
    этапе, а не только на воротах, и увести конвейер мимо оставшихся этапов
    может только ветка: узел роли остановку записал, но сам себя в обход
    топологии не отправит. На картинке графа это видно так же, как упор
    в бюджет, — и ровно поэтому перечислено ребром, а не спрятано в узле.
    ...
    Остановленный конвейер идёт мимо постлюдии: заводить задачи по
    недописанному backlog'у нельзя — тот же довод, что и у ветки бюджета.
    """
...
def halted_node(state: State, pipeline: Pipeline = roles.PIPELINE) -> dict:
    """
    Конвейер остановлен оператором. Денег не тратит.

    Остановиться можно двумя способами, и оператору важно, каким именно.
    На воротах он отказался продолжать после этапа, документ которого только
    что прочитал. На паузе (`pause.py`) он остановил конвейер перед этапом,
    который ещё не выполнялся, — и сказать «после» про этот случай значило бы
    назвать в сообщении не тот этап: тот, что назван, как раз и не состоялся.
    """
    halt = state.get("halt") or {}
    stage = halt.get("stage") or ""
    title = pipeline.by_key(stage).title if stage in pipeline.keys else ""
    if not title:
        where = "в середине конвейера"
    elif halt.get("point") == "before":
        where = f"на паузе перед этапом «{title}»"
    else:
        where = f"после этапа «{title}»"
    left = [role.title for role in pipeline.pending(state.get("artifacts"))]
    remaining = f" Не выполнены этапы: {', '.join(left)}." if left else ""
    return {
        "messages": [
            AIMessage(
                content=(
                    f"Конвейер остановлен оператором {where}. "
