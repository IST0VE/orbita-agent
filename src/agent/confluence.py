...
# Разметка
#
# Конвертер текста в storage format переехал в `confluence_storage.py`: здесь
# транспорт и операции над страницей, там — разметка. Имена остаются ради
# вызывающих: на `confluence.text_to_storage` и `confluence.mask_text`
# ссылаются рендерер, документы и половина тестов.
# --------------------------------------------------------------------------
from agent.confluence_storage import (  # noqa: E402, F401
    expand_macro,
    mask_text,
    text_to_storage,
)
