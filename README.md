...
Для графа `nt`: [настройка метрик и анализ завершённого НТ](docs/NT.md). Реализован MVP 1; запуск нагрузки и live monitoring относятся к следующим этапам. Вердикт по SLA считает код по серверной карте метрик; при явном разрешении модель может запросить метрику вне этой карты — код проверяет запрос, оператор подтверждает выполнение, и такой ряд идёт только в улики.
