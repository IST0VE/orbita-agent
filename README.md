...
| **Нагрузочное тестирование** | Есть завершённое НТ и метрики Prometheus/InfluxDB | Baseline, SLA verdict, аномалии, ranking и отчёт с гипотезами причин |

Для каждого сценария есть [инструкция с готовым запросом](docs/WORKFLOWS.md).

Для графа `nt`: [настройка метрик и анализ завершённого НТ](docs/NT.md). Реализован анализ завершённых тестов: отдельная оценка SLA и полноты диагностики, плато нагрузки и сопоставление прогонов. Запуск нагрузки и live monitoring в этот граф не входят. Вердикт по SLA считает код по серверной карте метрик; при явном разрешении модель может запросить метрику вне этой карты — код проверяет запрос, оператор подтверждает выполнение, и такой ряд идёт только в улики.
