...
export const GraphCanvas = memo(function GraphCanvas({ topology, manifest, runtime, selected, onSelect }: {
  ...
  return <div className="engine-canvas">
    ...
    {mode === "list" ? <table className="graph-list"><thead><tr><th>Узел</th><th>Тип</th><th>Статус</th><th>Запуски</th></tr></thead><tbody>{topology.nodes.filter((node) => visible(node.id)).map((node) => <tr key={node.id} tabIndex={0} className={selected === node.id ? "selected" : ""} onClick={() => onSelect(node.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(node.id); } }}><td>{manifest.nodes?.[node.id]?.title ?? node.id}<small>{node.id}</small></td><td>{manifest.nodes?.[node.id]?.kind ?? "task"}</td><td>{nodeStatus(runtime, node.id)}</td><td>{visitCount(runtime, node.id)}</td></tr>)}</tbody></table> : null}
    {mode === "visual" ? <div className="canvas-viewport"><svg width={width * zoom} height={height * zoom} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Интерактивная схема графа"><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" /></marker></defs>{topology.edges.map((edge, index) => {
      ...
      return <g key={node.id} transform={`translate(${position.x} ${position.y})`} className={`canvas-node node-${status} kind-${info?.kind ?? "task"} ${selected === node.id ? "selected" : ""}`} role="button" tabIndex={0} aria-label={`${info?.title ?? node.id}: ${status}`} onClick={() => onSelect(node.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(node.id); } }}><rect width="160" height="56" rx="4" /><text x="80" y="23" textAnchor="middle">{(info?.title ?? node.id).slice(0, 24)}</text><text x="80" y="42" textAnchor="middle" className="node-meta">{status}{count > 1 ? ` ×${count}` : ""}</text></g>;
